...
void buscaPalavra(MultiLista *ml){
     ...
     if(multiListaVazia(ml)==NAO){
          while(auxLinhas != NULL ){
            auxColunas = auxLinhas -> primeiro;
              while(auxColunas != NULL){
                if(auxColunas -> dado.palavra == word){
                     linha = auxColunas -> linha;
                     coluna = auxColunas -> coluna;
                     break;
                }else auxColunas = auxColunas -> proximo;
              }
              auxLinhas = auxLinhas -> abaixo;
    ...
    if(linha== -1 || coluna==-1) puts("Palavra nao encontrada na multilista");
    else printf("Linhas: %d\nColunas: %d\n", linha, coluna);
...
/*void removePalavraEspecifica (MultiLista *ml, int linha, int coluna){ // Remove a palavra da linha e da coluna especifica
    ...
    if(auxnodo->proximo != NULL){
        auxnodo2->proximo=auxnodo->proximo;
    }
    free(auxnodo);
}*/
