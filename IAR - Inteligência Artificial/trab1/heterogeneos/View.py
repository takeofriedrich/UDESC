...
class View:
    ...
    def __init__(self, path=None) -> None:
        if(View.tipo == 0):
            View.__labels = [(255, 155, 0), (255, 0, 255),
                             (0, 0, 255), (0, 255, 0), ]
        else:
            View.__labels = [(255, 0, 0),
                             (128, 128, 128),
                             (128, 128, 0),
                             (128, 0, 0),
                             (0, 128, 0),
        ...
        View.__simulation = Simulation(
            era=1, dados=ParserDados.parseDados(path) if path is not None else None, n=50 if View.tipo == 0 else 61, m=50 if View.tipo == 0 else 61)
    ...
    def __take_screenshot(filename):
        image = pygame.Surface(
            (500 if View.tipo == 0 else 600, 500 if View.tipo == 0 else 600))
        image.blit(View.__screen, (0, 0), ((0, 0), (500 if View.tipo ==
                   0 else 600, 500 if View.tipo == 0 else 600)))
    ...
    def __run():
        ...
        while 1:
            ...
            if(pygame.key.get_pressed()[K_0]):
                image = pygame.Surface(
                    (500 if View.tipo == 0 else 600, 500 if View.tipo == 0 else 600))
                image.blit(View.__screen, (0, 0), ((
                    0, 0), (500 if View.tipo == 0 else 600, 500 if View.tipo == 0 else 600)))
                pygame.image.save(
            ...
            if View.__simulation.countCarring() == 0:
                View.__ociosas += 1
            else:
                View.__ociosas = 0
            ...
            if(((age-1) % 1000) == 0):
                ...
                View.__take_screenshot(
                    'screenshots/{}-raio-{}-k1-{}-k2-{}-age-{}.png'.format('Dados4' if View.tipo == 0 else 'Dados15', Simulation.ray, Simulation.k1, Simulation.k2, age))

            if age == 30000 or View.__ociosas == 1000:
                Simulation.stop = not Simulation.stop
            if (age > 30000 and View.__simulation.countCarring() == 0) or (View.__ociosas > 10 and Simulation.stop):
                print('taking FINAL')
                View.__take_screenshot(
                    'screenshots/{}-raio-{}-k1-{}-k2-{}-age-{}.png'.format('Dados4' if View.tipo == 0 else 'Dados15', Simulation.ray, Simulation.k1, Simulation.k2, age))
                return
    ...
    def standalone():
        pygame.init()
        View.__screen = pygame.display.set_mode(
            (500 if View.tipo == 0 else 600, 500 if View.tipo == 0 else 600), DOUBLEBUF)
        View.__run()
        pygame.quit()
